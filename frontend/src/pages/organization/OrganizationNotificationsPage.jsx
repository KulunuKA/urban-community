...
export default function OrganizationNotificationsPage() {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      const errorMessage =
        fetchError?.response?.data?.message || "Failed to load notifications";
  ...
  return (
    <div className="mx-auto w-full max-w-5xl px-6 py-8">
      <header className="mb-6">
        <h1 className="text-3xl font-bold text-slate-900">Organization Notifications</h1>
        <p className="mt-2 text-sm text-slate-600">
          See updates about event join requests and response activity.
        </p>
      </header>
